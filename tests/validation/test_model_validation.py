"""
Validation tests for AI predictions vs actual outcomes
"""
import pytest
import time
import json
...
from historical_tracker import HandResult
...
class TestModelValidation:
    ...
    def test_blackjack_strategy_validation(self, blackjack_odds_calculator, validation_scenarios):
            ...
            if odds:
                ...
                if scenario['confidence_threshold'] > 0:
                    assert prediction == expected, \
                        f"Strategy error in {scenario['name']}: predicted {prediction}, expected {expected}"
        ...
        self._save_validation_results('blackjack_strategy_validation', validation_results)
    ...
    def test_poker_equity_validation(self, poker_odds_calculator, validation_scenarios):
        ...
        for scenario in validation_scenarios:
            ...
            if odds:
                ...
                assert equity >= expected_min, \
                    f"Equity too low in {scenario['name']}: {equity:.3f} < {expected_min:.3f}"
        ...
        # Save validation results
        self._save_validation_results('poker_equity_validation', validation_results)
    ...
    def test_monte_carlo_accuracy_validation(self, poker_odds_calculator):
        ...
        for scenario in test_scenarios:
            ...
            assert expected_min <= mean_equity <= expected_max, \
                f"Mean equity {mean_equity":.3f} not in expected range {scenario['expected_range']} for {scenario['name']}"

            # Validate that standard deviation is reasonable (not too much variance)
            assert std_dev < 0.1, \
                f"High variance in {scenario['name']}: std_dev = {std_dev":.3f}"

        # Save detailed validation results
        self._save_validation_results('monte_carlo_accuracy_validation', validation_results)
    ...
    def test_card_counting_accuracy_validation(self, blackjack_odds_calculator):
        ...
        for sequence in test_sequences:
            ...
            result = {
                ...
                'correct_sign': (true_count > 0) == sequence['expected_positive'],
                'timestamp': time.time()
            }
        ...
        # Save validation results
        self._save_validation_results('card_counting_validation', validation_results)

        print("Card counting validation completed successfully")

    @pytest.mark.validation
    @pytest.mark.slow
    def test_long_term_accuracy_tracking(self, database_manager, sample_config):
        """Test long-term accuracy tracking and validation"""
        # This test simulates long-term usage and validates that
        # the system maintains accuracy over many hands

        calc = BlackjackOddsCalculator(sample_config.game.blackjack_rules)

        # Simulate 100 hands with known outcomes
        test_hands = []
        predictions = []
        actual_outcomes = []

        for i in range(100):
            # Create a deterministic hand based on index
            hand_type = i % 4
            if hand_type == 0:
                # Strong hand (20 vs 6)
                cards = [BlackjackCard('K', 'hearts', 10), BlackjackCard('Q', 'spades', 10)]
                dealer_card = BlackjackCard('6', 'diamonds', 6)
                expected_action = 'stand'
            elif hand_type == 1:
                # Drawing hand (16 vs 10)
                cards = [BlackjackCard('10', 'hearts', 10), BlackjackCard('6', 'spades', 6)]
                dealer_card = BlackjackCard('10', 'diamonds', 10)
                expected_action = 'hit'
            elif hand_type == 2:
                # Pair (A,A vs 6)
                cards = [BlackjackCard('A', 'hearts', 11), BlackjackCard('A', 'spades', 11)]
                dealer_card = BlackjackCard('6', 'diamonds', 6)
                expected_action = 'split'
            else:
                # Soft hand (A,7 vs 2)
                cards = [BlackjackCard('A', 'hearts', 11), BlackjackCard('7', 'spades', 7)]
                dealer_card = BlackjackCard('2', 'diamonds', 2)
                expected_action = 'stand'
            ...
            game_state = TestUtils.create_mock_game_state(
                ...
                player_score=sum(card.value for card in cards)
            )
            ...
            if odds:
                ...
                test_hands.append(game_state)
                predictions.append(prediction)
                actual_outcomes.append(expected_action)
                ...
                hand_result = HandResult(
                    ...
                    actual_outcome=expected_action,
                    expected_value=0.5,  # Mock expected value
                    actual_profit_loss=10 if prediction == expected_action else -10,
                    game_state_summary={'player_score': game_state.player_score},
                    confidence_score=0.8
                )
                database_manager.insert_hand_result(hand_result)

        # Calculate accuracy
        accuracy = TestUtils.calculate_expected_accuracy(predictions, actual_outcomes)

        print(f"Long-term accuracy: {accuracy":.2%} ({sum(1 for p, a in zip(predictions, actual_outcomes) if p == a)}/{len(predictions)})")

        # Validate database storage
        performance = database_manager.get_model_performance('blackjack')
        assert performance['total_hands'] >= 90  # Should have recorded most hands
        assert performance['accuracy'] >= 0.8   # Should meet accuracy threshold

        # Save long-term validation results
        long_term_results = {
            'accuracy': accuracy,
            'total_hands': len(predictions),
        ...
        self._save_validation_results('long_term_accuracy_validation', long_term_results)

        # Long-term accuracy should meet minimum threshold
        assert accuracy >= 0.85, f"Long-term accuracy {accuracy":.2%} below 85% threshold"
    ...
    def _save_validation_results(self, test_name: str, results: Any):
        """Save validation results for analysis"""
        try:
            filename = f"validation_results_{test_name}_{int(time.time())}.json"
    ...
    @pytest.mark.validation
    @pytest.mark.slow
    def test_performance_under_load(self, poker_odds_calculator, blackjack_odds_calculator):
        """Test system performance under load"""
        import time

        # Test poker performance under load
        poker_start = time.time()
        for _ in range(100):
            state = TestUtils.create_mock_game_state(
                'poker',
                player_cards=[Card('A', 'hearts'), Card('K', 'spades')],
                community_cards=[]
            )
            poker_odds_calculator.calculate_odds(state)

        poker_time = time.time() - poker_start

        # Test blackjack performance under load
        blackjack_start = time.time()
        for _ in range(100):
            state = TestUtils.create_mock_game_state(
                'blackjack',
                player_cards=[BlackjackCard('A', 'hearts', 11), BlackjackCard('K', 'spades', 10)],
            ...
            blackjack_odds_calculator.calculate_odds(state)

        blackjack_time = time.time() - blackjack_start

        # Performance assertions
        assert poker_time < 30, f"Poker calculations too slow: {poker_time":.2f}s for 100 hands"
        assert blackjack_time < 30, f"Blackjack calculations too slow: {blackjack_time":.2f}s for 100 hands"

        print(f"Performance under load - Poker: {poker_time":.2f}s, Blackjack: {blackjack_time":.2f}s")

        # Save performance results
        performance_results = {
            'poker_time': poker_time,
            'blackjack_time': blackjack_time,
            'poker_avg_time': poker_time / 100,
            'blackjack_avg_time': blackjack_time / 100,
            'timestamp': time.time()
        }

        self._save_validation_results('performance_under_load', performance_results)

    @pytest.mark.validation
    def test_memory_usage_validation(self, poker_odds_calculator, blackjack_odds_calculator):
        ...
        for _ in range(50):
            ...
            poker_odds_calculator.calculate_odds(poker_state)
            ...
            blackjack_odds_calculator.calculate_odds(blackjack_state)

        final_memory = process.memory_info().rss / 1024 / 1024  # MB
        memory_increase = final_memory - initial_memory

        # Memory increase should be reasonable (< 50MB)
        assert memory_increase < 50, f"Memory usage increased by {memory_increase":.1f}MB, which seems excessive"

        print(f"Memory usage validation - Initial: {initial_memory":.1f}MB, Final: {final_memory":.1f}MB, Increase: {memory_increase":.1f}MB")
        ...
        self._save_validation_results('memory_usage_validation', memory_results)
